...
def create_features(df):
    # DEBUG: inspect columns at the exact crash point
    st.write("🧱 create_features df.columns:", list(df.columns))
    st.write("🧱 create_features sample:", df.head())

    # HARD CHECK: fail fast if off_wepa_play is missing
    if "off_wepa_play" not in df.columns:
        st.write("❌ MISSING COLUMN: off_wepa_play")
    if "off_wepa_play_opp" not in df.columns:
        st.write("❌ MISSING COLUMN: off_wepa_play_opp")
...
def get_week_picks_singleweek(week_number):
    ...
    st.write("📄 spreads.columns:", list(spreads.columns))
    st.write("📄 spreads sample:", spreads.head())
    ...
    st.write("🧬 games.columns after merge:", list(games.columns))
    st.write("🧬 games sample after merge:", games.head())
