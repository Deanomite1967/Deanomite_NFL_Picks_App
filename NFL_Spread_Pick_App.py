...
def add_recommended_pick(df):
    ...
    for _, row in df.iterrows():
        ...
        # Team-side adjustment
        adj_team = row.get("Adj_value", 0.0)

        # Opponent-side adjustment
        adj_opp = row.get("Adj_value_opp", 0.0)

        # Adjust model_pred based on which side the adjustment belongs to
        model_adj = row["model_pred"] + adj_team - adj_opp
