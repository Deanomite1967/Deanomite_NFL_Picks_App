...
def load_week_spreads(path):
    df = pd.read_csv("Week1_Spreads.csv")
...
def predict_games(model, df, feature_cols):
    ...
    df["edge"] = (df["model_pred"] + df.get("Adj_value", 0.0)) - df["spread_value"]
    return df
...
def add_recommended_pick(df):
    ...
    for _, row in df.iterrows():
        ...
        vegas = row["spread_value"]          # ALWAYS use Vegas spread
        adj = row.get("Adj_value", 0.0)      # Adjustment applies ONLY to model_pred
        model_adj = row["model_pred"] + adj  # Adjusted model prediction
...
confidence_filter = st.sidebar.multiselect(
    ...
    ["Bet Favorite", "Bet Underdog", "Lean Favorite", "Lean Underdog", "No Model Edge"],
    default=["Bet Favorite", "Bet Underdog", "Lean Favorite", "Lean Underdog", "No Model Edge"]
)
