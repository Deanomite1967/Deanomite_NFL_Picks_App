...
def merge_matchups(games, team_master):
    ...
    # Drop ANY team columns safely
    for col in df.columns:
        if col.startswith("team"):
            df = df.drop(columns=[col], errors="ignore")

    # Second merge: Opp → team_opp
    df = df.merge(
    ...
    # Drop ANY team_opp columns safely
    for col in df.columns:
        if col.startswith("team_opp"):
            df = df.drop(columns=[col], errors="ignore")
...
def create_features(df):
    st.write("🧱 create_features df.columns:", list(df.columns))

    # --- Normalize possible suffixed columns to canonical names ---
    rename_map = {}

    # Offense EPA
    if "off_wepa_play" not in df.columns:
        if "off_wepa_play_x" in df.columns:
            rename_map["off_wepa_play_x"] = "off_wepa_play"
        elif "off_wepa_play_y" in df.columns:
            rename_map["off_wepa_play_y"] = "off_wepa_play"

    # Defense EPA
    if "def_wepa_play" not in df.columns:
        if "def_wepa_play_x" in df.columns:
            rename_map["def_wepa_play_x"] = "def_wepa_play"
        elif "def_wepa_play_y" in df.columns:
            rename_map["def_wepa_play_y"] = "def_wepa_play"

    # Opponent Offense EPA
    if "off_wepa_play_opp" not in df.columns:
        if "off_wepa_play_opp_x" in df.columns:
            rename_map["off_wepa_play_opp_x"] = "off_wepa_play_opp"
        elif "off_wepa_play_opp_y" in df.columns:
            rename_map["off_wepa_play_opp_y"] = "off_wepa_play_opp"

    # Opponent Defense EPA
    if "def_wepa_play_opp" not in df.columns:
        if "def_wepa_play_opp_x" in df.columns:
            rename_map["def_wepa_play_opp_x"] = "def_wepa_play_opp"
        elif "def_wepa_play_opp_y" in df.columns:
            rename_map["def_wepa_play_opp_y"] = "def_wepa_play_opp"

    # Success rate
    if "off_sr" not in df.columns:
        if "off_sr_x" in df.columns:
            rename_map["off_sr_x"] = "off_sr"
        elif "off_sr_y" in df.columns:
            rename_map["off_sr_y"] = "off_sr"

    if "def_sr" not in df.columns:
        if "def_sr_x" in df.columns:
            rename_map["def_sr_x"] = "def_sr"
        elif "def_sr_y" in df.columns:
            rename_map["def_sr_y"] = "def_sr"

    if "off_sr_opp" not in df.columns:
        if "off_sr_opp_x" in df.columns:
            rename_map["off_sr_opp_x"] = "off_sr_opp"
        elif "off_sr_opp_y" in df.columns:
            rename_map["off_sr_opp_y"] = "off_sr_opp"

    if "def_sr_opp" not in df.columns:
        if "def_sr_opp_x" in df.columns:
            rename_map["def_sr_opp_x"] = "def_sr_opp"
        elif "def_sr_opp_y" in df.columns:
            rename_map["def_sr_opp_y"] = "def_sr_opp"

    # Trenches
    if "ol_rate" not in df.columns:
        if "ol_rate_x" in df.columns:
            rename_map["ol_rate_x"] = "ol_rate"
        elif "ol_rate_y" in df.columns:
            rename_map["ol_rate_y"] = "ol_rate"

    if "dl_rate" not in df.columns:
        if "dl_rate_x" in df.columns:
            rename_map["dl_rate_x"] = "dl_rate"
        elif "dl_rate_y" in df.columns:
            rename_map["dl_rate_y"] = "dl_rate"

    if "ol_rate_opp" not in df.columns:
        if "ol_rate_opp_x" in df.columns:
            rename_map["ol_rate_opp_x"] = "ol_rate_opp"
        elif "ol_rate_opp_y" in df.columns:
            rename_map["ol_rate_opp_y"] = "ol_rate_opp"

    if "dl_rate_opp" not in df.columns:
        if "dl_rate_opp_x" in df.columns:
            rename_map["dl_rate_opp_x"] = "dl_rate_opp"
        elif "dl_rate_opp_y" in df.columns:
            rename_map["dl_rate_opp_y"] = "dl_rate_opp"

    # Red zone blended
    if "Off_rz" not in df.columns:
        if "Off_rz_x" in df.columns:
            rename_map["Off_rz_x"] = "Off_rz"
        elif "Off_rz_y" in df.columns:
            rename_map["Off_rz_y"] = "Off_rz"

    if "Def_rz" not in df.columns:
        if "Def_rz_x" in df.columns:
            rename_map["Def_rz_x"] = "Def_rz"
        elif "Def_rz_y" in df.columns:
            rename_map["Def_rz_y"] = "Def_rz"

    if "Off_rz_opp" not in df.columns:
        if "Off_rz_opp_x" in df.columns:
            rename_map["Off_rz_opp_x"] = "Off_rz_opp"
        elif "Off_rz_opp_y" in df.columns:
            rename_map["Off_rz_opp_y"] = "Off_rz_opp"

    if "Def_rz_opp" not in df.columns:
        if "Def_rz_opp_x" in df.columns:
            rename_map["Def_rz_opp_x"] = "Def_rz_opp"
        elif "Def_rz_opp_y" in df.columns:
            rename_map["Def_rz_opp_y"] = "Def_rz_opp"

    # Explosive / YPP / SOS
    for base in ["off_explosive", "off_ypp", "off_sos"]:
        ...
        if opp not in df.columns:
            if f"{opp}_x" in df.columns:
                rename_map[f"{opp}_x"] = opp
            elif f"{opp}_y" in df.columns:
                rename_map[f"{opp}_y"] = opp

    df = df.rename(columns=rename_map)

    # Final sanity check
    required = [
        "off_wepa_play", "off_wepa_play_opp",
        "def_wepa_play", "def_wepa_play_opp",
        "off_sr", "off_sr_opp",
        "def_sr", "def_sr_opp",
        "ol_rate", "dl_rate",
        "ol_rate_opp", "dl_rate_opp",
        "Off_rz", "Off_rz_opp",
        "Def_rz", "Def_rz_opp",
        "off_explosive", "off_explosive_opp",
        "off_ypp", "off_ypp_opp",
        "off_sos", "off_sos_opp",
        "spread_value"
    ]
    ...
    if missing:
        st.write("❌ STILL MISSING REQUIRED COLUMNS:", missing)
        st.stop()

    # --- Original feature logic ---
...
def run_picking_machine(games, team_master):
    # DEBUG: what does games look like before features?
    st.write("🔎 games.columns:", list(games.columns))
    st.write("🔎 sample games rows:", games.head())
...
def build_training_data(week_number):
    ...
    # Ensure spreads has spread_value BEFORE merging
    spreads["spread_value"] = pd.to_numeric(spreads["Spread"], errors="coerce")
    ...
    # --- FIX spread_value after merge ---
    if "spread_value_x" in df.columns:
        df = df.rename(columns={"spread_value_x": "spread_value"})
    elif "spread_value_y" in df.columns:
        df = df.rename(columns={"spread_value_y": "spread_value"})
...
def get_week_picks_singleweek(week_number):
    ...
    # ONLY ONE MERGE — via merge_matchups
    games = merge_matchups(spreads, team_master_blended)
